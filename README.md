...
Chat Games can award passive points to logged-in accounts connected to the broadcaster's Twitch chat, including lurkers who have not spoken, using Twitch's delayed Get Chatters list. Anonymous video viewers and viewers who are not connected to chat cannot be identified. If the chatter lookup is disabled or temporarily unavailable, recently active speakers remain eligible. Its points have no cash value. Plugin configuration is included in Forge's settings export, while live balances and timer state remain portable in the profile's plugin-data folder and survive updates.
